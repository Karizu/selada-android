...
typedef struct contactless_card_hal_interface {
	...
	contactless_card_final_touch_card touch_card;
	int pCardHandle;
	void* pSoHandle;
	//sem_t sem;
...
jint JNICALL native_contactless_card_open(JNIEnv* env, jclass obj) {
	...
	if (g_pContactlessCard == NULL) {
		...
		if (NULL== (g_pContactlessCard->open = (contactless_card_open) dlsym(pSoHandle,methodName = "contactless_card_open"))
				...
				|| NULL== (g_pContactlessCard->transmit = (contactless_card_transmit) dlsym(pSoHandle,methodName = "contactless_card_transmit"))
		...
		if (NULL== (g_pContactlessCard->query_info = (contactless_card_query_info) dlsym(pSoHandle,methodName = "contactless_card_query_info"))
				...
				|| NULL== (g_pContactlessCard->touch_card = (contactless_card_final_touch_card) dlsym(pSoHandle,methodName = "contactless_card_final_touch_card")))
		{
			hal_sys_error("can't find %s", methodName);
		}
...
jint JNICALL native_contactless_card_final_touch_card(JNIEnv* env, jclass obj) {
	hal_sys_info("+ native_contactless_card_final_touch_card()");
	int nResult = ERR_NORMAL;
	if (g_pContactlessCard == NULL)
		return ERR_NOT_OPENED;
	if (g_pContactlessCard->touch_card == NULL)
		return ERR_NO_IMPLEMENT;

	nResult = g_pContactlessCard->touch_card(g_pContactlessCard->pCardHandle);
	hal_sys_info("- native_contactless_card_final_touch_card(), result = %d", nResult);
	return nResult;
}
