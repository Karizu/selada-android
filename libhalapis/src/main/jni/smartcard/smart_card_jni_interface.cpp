...
static void smart_card_event_notify(void* pUserData, int nSlotIndex,
	...
	env->CallStaticVoidMethod(mcls, mmid, nSlotIndex, nEvent);
...
jint JNICALL native_smart_card_open(JNIEnv* env, jclass obj, jint nSlotIndex) {
	...
	if (g_pSmartCardInstance == NULL) {
		...
		//		+ add by pengli
		mmid = env->GetStaticMethodID(mcls, "callBack", "(II)V");
		if (mmid == NULL) {
			return ERR_NORMAL;
		}
	}
	nResult = g_pSmartCardInstance->open(nSlotIndex, smart_card_event_notify,
			g_pSmartCardInstance);
...
jint JNICALL native_smart_card_touch(JNIEnv* env, jclass obj, jint nHandle) {
	...
	if (g_pSmartCardInstance == NULL)
		return ERR_NOT_OPENED;
	if(g_pSmartCardInstance->touch == NULL)
		return ERR_NO_IMPLEMENT;
	nResult = g_pSmartCardInstance->touch(nHandle);
	hal_sys_info("- native_smart_card_touch(),result = %d", nResult);
	return nResult;
}
